...
def block_user(request,id):
    print(id)
    aa = Login.objects.get(id=id)
    aa.type = 'blocked'
    aa.save()
    return HttpResponse('''<script>window.location='/view_users'</script>''')

def un_user(request,id):
    aa=Login.objects.get(id=id)
    aa.type='user'
    aa.save()
    return HttpResponse('''<script>window.location='/view_users'</script>''')
...
def user_edit_profile(request):
    a=User.objects.get(LOGIN_id=request.session['lid'])
    return render(request,'user/edituserprofile.html',{'profile':a})
...
def user_edit_profile_post(request):
    ...
    if 'submit' in request.POST:
        ...
        b = User.objects.get(LOGIN__id=request.session['lid'])
        # b.LOGIN = a
        b.name = name
        b.phone = phone
        b.place = place
...
def delete_h_booking(request,id):
    if request.session['lid'] == '':
        return HttpResponse('''<script>alert("Logout successfully ");window.location='/'</script>''')

    users = Hotel_Booking.objects.get(id=id)  # Fetch all user records
    users.delete()
    return redirect("/user_view_hotel_booking")
...
def delete_pakage_booking(request,id):
    if request.session['lid'] == '':
        return HttpResponse('''<script>alert("Logout successfully ");window.location='/'</script>''')

    users = Package_Booking.objects.get(id=id)  # Fetch all user records
    users.delete()
    return redirect("/user_view_package_booking")
...
def guide_view_user(request):
    if request.session['lid'] == '':
        return HttpResponse('''<script>alert("Logout successfully ");window.location='/'</script>''')
    b=Guide_Booking.objects.filter(GUIDE__LOGIN__id=request.session['lid'])
    aa=[]
    for i in b:
        aa.append(i.USER.id)
    a=User.objects.filter(id__in=aa)
...
def delete_guid_booking(request,id):
    a=Guide_Booking.objects.get(id=id)
    a.delete()
    return redirect("/user_view_guide_booking")
